...
int determineShift(int value) {
    int shift = 0;
    while (value > 1) {
        value >>= 1; // Shift the value right by one bit
        shift++;
    }
    return shift - (shift % 4); // Adjust to the nearest multiple of 4 if necessary
}
...
void LCD_GPIO_Init(LCD_Config * lcdConfig)
{
	...
	{
		if(lcdConfig->dataPins == LCD_DPINS_LOWER_HALF)
		{
...
//	#ifdef EIGHT_BIT_MODE
	...
	if(lcdConfig->LCD_Mode == LCD_MODE_8BIT)
	{
		if(lcdConfig->dataPins == LCD_DPINS_LOWER_HALF)
		{
			MCAL_GPIO_WritePartialPort(lcdConfig->LCD_dataPort, GPIO_PIN_0 , GPIO_PIN_7, command);
		}
		else
		{
			MCAL_GPIO_WritePartialPort(lcdConfig->LCD_dataPort, GPIO_PIN_8 , GPIO_PIN_15, command);
		}
	}
	else //4 bit mode
	{
		uint8_t shiftpos = determineShift(lcdConfig->dataPins);
...
//	8Bit mode
	if(lcdConfig->LCD_Mode == LCD_MODE_8BIT)
	{
		if(lcdConfig->dataPins == LCD_DPINS_LOWER_HALF)
		{
			MCAL_GPIO_WritePartialPort(lcdConfig->LCD_dataPort, GPIO_PIN_0 , GPIO_PIN_7, (uint16_t)character);
		}
		else
		{
			MCAL_GPIO_WritePartialPort(lcdConfig->LCD_dataPort, GPIO_PIN_8 , GPIO_PIN_15, character);
		}
...
void HAL_LCD_Kick(LCD_Config * lcdConfig) {
    ...
    My_Wait(40); // Increase this delay as needed
    MCAL_GPIO_WritePin(lcdConfig->LCD_ctrlPort, EN_Pos, GPIO_PIN_STATUS_LOW);
